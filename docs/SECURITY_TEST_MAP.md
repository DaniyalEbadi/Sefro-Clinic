...
| A04 Insecure Design | Brute-force throttle test, overlap double-booking prevention (`tests/integration/test_visit_overlap.py`); public Face AI upload surface throttled via `FACE_ANALYZER_THROTTLE_RATE` (`tests/integration/test_face_analyzer.py::FaceAnalyzeEndpointTests::test_analyze_throttle_429`, 6th request → 429); residual design risks recorded as **accepted** by the owner (see Accepted Risks below) | ✅ accepted-risk |
...
| A10 Server-Side Request Forgery | Outbound HTTP now exists: external face analyzer (`FACE_ANALYZER_EXTERNAL_API_URL`, server-configured only — no user-supplied URLs) and optional LLM tips provider (`FACE_ANALYZER_LLM_API_KEY`, also server-configured). URLs/keys come from `.env`, never from request data; both are empty/disabled by default. Revisit if user-supplied URLs are ever added | ✅ config-only |
