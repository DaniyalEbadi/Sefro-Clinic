...
from finance.models import Wallet, WalletRewardRule, Sale, ExpenseCategory, Expense, Expense as ExpenseModel, WalletTransaction
from inventory.models import Product
from finance.services import accounting, payments, reporting, expenses as expense_svc
from finance.services.exchange_rates import get_rate, set_rate, to_toman
from finance.services.wallet import grant_reward, reverse_reward, credit, debit, current_balance, InsufficientFunds
...
class PaymentsServiceTests(TestCase):
    def setUp(self):
        set_rate('USD', 'TOMAN', Decimal('100000'), effective_at=timezone.now(), source='test')
        self.customer = make_customer()
        self.wallet = Wallet.objects.create(customer=self.customer, balance=Decimal('40'))
    ...
    def test_checkout_mixed_payment(self):
        ...
        self.assertTrue(
            WalletTransaction.objects.filter(transaction_type='payment', amount=Decimal('-40.00')).exists()
        )
        self.assertTrue(
    ...
    def test_wallet_refund_restores_balance(self):
        ...
        # Wallet already has 100 from setUp credit
...
class ReportingServiceTests(TestCase):
    ...
    def test_financial_summary(self):
        ...
        exp = expense_svc.create_expense(
            created_by=make_admin(), category=cat, amount_usd=Decimal('30'), expense_date=timezone.now().date(), vendor='Shop',
        )
        expense_svc.submit_expense(exp)
        expense_svc.approve_expense(exp, make_admin())
...
class InventoryServiceTests(TestCase):
    ...
    def test_cost_history_and_snapshot(self):
        ...
        self.assertEqual(usage2.total_cost_usd_snapshot, Decimal('20.00'))
